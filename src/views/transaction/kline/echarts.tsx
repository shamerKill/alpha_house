...
const MarketKlineHtml: FC<{style: StyleProp<ViewStyle>}> = ({
  ...
  useEffect(() => {
    ...
    const getKLineDataTransactionByLength = async (time: number, coinTypeIn: string) => {
    ...
    const inter = setInterval(() => {
      if (viewShowRef.current) {
        getKLineDataTransactionByLength(100, coinType).then(data => {
          if (data) {
            getKLineDataTransactionByLength(500, coinType).then((res) => {
              if (res) {
